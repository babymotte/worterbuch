...
local worterbuch_proto = Proto("worterbuch", "Wörterbuch Protocol")

-- Protocol fields
local f_message = ProtoField.string("worterbuch.message", "JSON Message")
local f_partial = ProtoField.string("worterbuch.partial", "Partial Message")
local f_message_type = ProtoField.string("worterbuch.message_type", "Message Type")
local f_direction = ProtoField.string("worterbuch.direction", "Direction")
local f_transaction_id = ProtoField.string("worterbuch.transaction_id", "Transaction ID")
local f_validation = ProtoField.string("worterbuch.validation", "Validation")
local f_invalid = ProtoField.string("worterbuch.invalid", "Invalid Message")

worterbuch_proto.fields = {
    f_message, f_partial, f_message_type, f_direction, 
    f_transaction_id, f_validation, f_invalid
}

-- Preferences
worterbuch_proto.prefs.port = Pref.uint("TCP Port", 8081, "The TCP port for this protocol")
...
function worterbuch_proto.dissector(buffer, pinfo, tree)
    local buf_len = buffer:len()
    if buf_len == 0 then return end
    
    pinfo.cols.protocol = worterbuch_proto.name
    
    local subtree = tree:add(worterbuch_proto, buffer(), "Wörterbuch Protocol")
    
    -- Determine direction based on port
    local is_client_to_server = (pinfo.dst_port == worterbuch_proto.prefs.port)
...
tcp_table:add(worterbuch_proto.prefs.port, worterbuch_proto)

-- Allow dynamic port changes
function worterbuch_proto.init()
    tcp_table:add(worterbuch_proto.prefs.port, worterbuch_proto)
end