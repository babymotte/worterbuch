...
async fn main() -> Result<()> {
    let (matches, proto, host_addr, port, json) = app(
        "wbpub",
        "Publish values on a Wörterbuch.",
        vec![
            Arg::with_name("KEY_VALUE_PAIRS")
                .multiple(true)
                .help(
                    r#"Key/value pairs to be published on Wörterbuch in the form "KEY1=VALUE1 KEY2=VALUE2 KEY3=VALUE3 ...". When omitted, key/value pairs will be read from stdin. When reading key/value pairs from stdin, one key/value pair is expected per line."#,
                )
                .takes_value(true)
                .required(false)
        ],
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            match serde_json::from_str::<KeyValuePair>(&line) {
                Ok(KeyValuePair { key, value }) => {
                    trans_id = con.publish(key, json!(value))?;
                }
                Err(e) => {
        ...
        if let Some(key_calue_pairs) = key_value_pairs {
            for key_calue_pair in key_calue_pairs {
                if let Some(index) = key_calue_pair.find('=') {
                    ...
                    trans_id = con.publish(key.to_owned(), json!(val))?;
                } else {
