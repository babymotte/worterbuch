/*
 *  Worterbuch cli client for setting values
 *
...
#[derive(Parser)]
#[command(author, version, about = "Send a stream of values read from stdin to a single Wörterbuch key.", long_about = None)]
struct Args {
...
async fn run(subsys: Subsystem) -> Result<()> {
    ...
    subsys.spawn("provide_values", async move |s| {
    ...
    loop {
        ...
        select! {
            ...
            recv = next_item(&mut rx, done) => match recv {
                Some(value) => trans_id = wb.set_async(key.clone(), &value).await?,
                None => done = true,
            },
        }
    }
